...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   07.state6_to_state10.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: tmalheir <tmalheir@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/20 10:19:45 by tmalheir          #+#    #+#             */
/*   Updated: 2024/07/02 12:50:06 by tmalheir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	state_7(t_token_list **lst, char curr, char next, int idx)
{
	if (check_next_char(curr, next))
		idx = state_8(lst, idx, next);
	else
		idx = state_9(lst, idx, next);
	return (idx);
}
...
{
	if (check_next_char(curr, next))
		idx = state_11(lst, idx, next);
	else
		idx = state_12(lst, idx, next);
	return (idx);
}
